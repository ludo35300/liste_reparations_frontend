// ─────────────────────────────────────────────────────────────
// scan.ts — form migré en signal<ScanForm>
// ─────────────────────────────────────────────────────────────
...
import {
  ...
  faCheckCircle, faCirclePlus, faCheck, faFloppyDisk,
  faTimes, faPlus, faSearch, faLink
...
const EMPTY_FORM = (): ScanForm => ({
  ...
  technicien: '', technicien_id: null, date_reparation: '',
  date_affichage: '', notes: '', pieces: [],
});
...
export class Scan implements OnInit {
  ...
  readonly filteredPiecesModele = computed(() => {
    ...
    const f       = this.form();
    const dejaDans = new Set(f.pieces.map(p => p.ref_piece));
  ...
  readonly canSave = computed(() => {
    const f = this.form();
    if (!f.numero_serie?.trim())    return false;
    if (!f.date_reparation?.trim()) return false;
    if (!f.modele_id)               return false;
    if (this.saving())              return false;
    if (this.avertissementActif())  return false;

    return !f.pieces.some(piece => {
      if (piece.quantite <= 0) return false;
      const ref  = piece.ref_piece?.trim();
      const desig = piece.designation?.trim();
      if (!ref || !desig) return true;
  ...
  private getMarqueIdFromModele(): number | null {
    const modele = this.modeles.find(m => m.id === this.form().modele_id);
    ...
    this.compresserImage(this.selectedFile).then(compressed => {
      this.service.scanFiche(compressed).subscribe({
        next: (result: OcrResult) => {
          ...
          // Technicien
          let technicien_id = this.currentTechnicienId;
          let technicien    = this.techniciens.find(t => t.id === this.currentTechnicienId)?.nom ?? '';
          if (result.technicien) {
            const prenomOcr = result.technicien.trim().toLowerCase();
            const tech = this.techniciens.find(t => t.nom.trim().toLowerCase().startsWith(prenomOcr));
            if (tech) { technicien_id = tech.id; technicien = tech.nom; }
            else { technicien = this.techniciens.find(t => t.id === this.currentTechnicienId)?.nom ?? result.technicien; }
          }

          this.form.set({
            numero_serie:    result.numero_serie,
            date_reparation: this._normaliserDate(result.date),
            date_affichage:  this._formatPourAffichage(this._normaliserDate(result.date)),
          ...
          if (result.numero_serie) this._checkNumeroSerie(result.numero_serie);
  ...
  removePiece(index: number): void {
    ...
    this.form.update(f => ({ ...f, pieces: f.pieces.filter((_, i) => i !== index) }));
    if (ref) {
  ...
  addPieceFromModele(piece: PieceRef): void {
    if (this.form().pieces.find(p => p.ref_piece === piece.ref_piece)) return;
    this.form.update(f => ({
      ...f,
      pieces: [...f.pieces, { ref_piece: piece.ref_piece, designation: piece.designation, quantite: 1, is_new: false }],
    }));
  ...
  updateRef(index: number, value: string): void {
    this.form.update(f => ({
      ...f,
      pieces: f.pieces.map((p, i) => i === index ? { ...p, ref_piece: value.trim().toUpperCase() } : p),
    }));
  }
  ...
  updateDesignation(index: number, value: string): void {
    this.form.update(f => ({
      ...f,
      pieces: f.pieces.map((p, i) => i === index ? { ...p, designation: value } : p),
    }));
  }
  ...
  updateQuantite(index: number, value: string): void {
    this.form.update(f => ({
      ...f,
      pieces: f.pieces.map((p, i) => i === index ? { ...p, quantite: parseInt(value, 10) || 0 } : p),
    }));
  }
  ...
  validerPiece(index: number): void {
    ...
    if (!marqueId) { this.error.set('Impossible de créer la pièce : marque manquante.'); return; }
    ...
    this.refService.getAllPieces().pipe(
      switchMap(allPieces => {
        ...
        if (existing) {
          return this.form().modele_id
            ? this.refService.addPieceToModele(this.form().modele_id!, existing.id).pipe(catchError(() => of(null)))
            : of(null);
        }
        return this.refService.createPiece(piece.ref_piece, piece.designation, marqueId).pipe(
          switchMap(created => {
            this.piecesduModele.update(list => [...list, created]);
            return this.form().modele_id
              ? this.refService.addPieceToModele(this.form().modele_id!, created.id).pipe(catchError(() => of(null)))
              : of(null);
          })
        );
      }),
      ...
      if (modeleId) {
        this.refService.getPiecesByModele(modeleId).subscribe({ next: pieces => this.piecesduModele.set(pieces) });
      }
    });
  }
  ...
  associerPiece(piece: PieceChangee): void {
    ...
    this.refService.getAllPieces().pipe(
      switchMap(allPieces => {
        ...
        if (found) return this.refService.addPieceToModele(modeleId, found.id);
        const marqueId = this.form().marque_id ?? this.getMarqueIdFromModele();
        if (!marqueId) return of(null);
        return this.refService.createPiece(piece.ref_piece, piece.designation, marqueId).pipe(
          switchMap(created => this.refService.addPieceToModele(modeleId, created.id))
        );
      }),
      catchError(() => of(null))
    ).subscribe(() => {
  ...
  onDateInput(value: string): void {
    ...
    this.form.update(f => ({
      ...
      date_reparation: match ? `${match[3]}-${match[2]}-${match[1]}` : f.date_reparation,
    }));
  }
  ...
  async enregistrer(): Promise<void> {
    const f = this.form();
    if (!f.numero_serie || !f.date_reparation) {
    ...
    try {
      ...
      const payload: Reparation = {
        ...
        technicien_id:   f.technicien_id ?? this.currentTechnicienId ?? undefined,
        date_reparation: f.date_reparation,
        notes:           f.notes,
        pieces:          f.pieces.filter(p => p.quantite > 0),
      };
  ...
  private async _resoudreMachineId(): Promise<number> {
    const f = this.form();
    const numeroSerie = f.numero_serie.trim().toUpperCase();
    try {
      const result = await firstValueFrom(this.service.search(numeroSerie)) as any;
      if (result?.found && result?.machine?.id) return result.machine.id;
    } catch (err: any) {
      if (err?.status !== 404) throw err;
    }
    if (!f.modele_id) throw new Error('modele_id manquant — impossible de créer la machine.');
    const machine = await firstValueFrom(
      this.machineService.create({ numero_serie: numeroSerie, modele_id: f.modele_id, statut: 'en_attente', notes: f.notes ?? '' })
    );
    return machine.id;
  }
